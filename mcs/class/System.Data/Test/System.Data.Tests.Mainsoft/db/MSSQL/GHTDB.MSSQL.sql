...
if not exists (select * from master.dbo.syslogins where loginname = N'othertestuser')
BEGIN
	...
	exec sp_addlogin N'othertestuser', null, @logindb, @loginlang
END
GO

exec sp_addsrvrolemember N'othertestuser', sysadmin
GO


if not exists (select * from dbo.sysusers where name = N'othertestuser' and uid < 16382)
	EXEC sp_grantdbaccess N'othertestuser', N'othertestuser'
GO
...
if exists (select * from dbo.sysobjects where id = object_id(N'[othertestuser].[CategoriesNew]') and OBJECTPROPERTY(id, N'IsUserTable') = 1)
drop table [othertestuser].[CategoriesNew]
GO

CREATE TABLE [othertestuser].[CategoriesNew] (
	[CategoryID] [int] IDENTITY (1, 1) NOT NULL ,
	[CategoryName] [nvarchar] (15) COLLATE SQL_Latin1_General_CP1_CI_AS NOT NULL ,
	[Description] [ntext] COLLATE SQL_Latin1_General_CP1_CI_AS NULL ,
	[Picture] [image] NULL 
...
CREATE TABLE [othertestuser].[Categories] (
...
CREATE  procedure [othertestuser].[GH_DUMMY]
