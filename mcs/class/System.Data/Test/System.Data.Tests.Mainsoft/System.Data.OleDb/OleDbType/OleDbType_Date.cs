...
{
	[TestFixture] public class OleDbType_Date : GHTBase
	{
		...
		public void SetUp()
		{
			...
			try
			{
				dateTableName = DataProvider.EXTENDED_TYPES_TABLE_NAME;
		...
		public void run()
		{
			...
			try
			{
				...
				RetDate = System.Convert.ToDateTime(cmd.ExecuteScalar());
			...
			try
			{
				...
				if (DataProvider.GetDbType(con) == GHTUtils.DataBaseServer.SQLServer || DataProvider.GetDbType(con) == GHTUtils.DataBaseServer.Sybase)
				{
					cmd.CommandText = string.Format("select CONVERT(varchar,{0},120) from {1}  where ID='{2}'", dateColumnName, dateTableName, rowId);
					str = cmd.ExecuteScalar().ToString();
					Compare(TestedDate1, System.Convert.ToDateTime(str));
				}
				//' checking that value returned correctly
				cmd.CommandText =string.Format("select {0} from {1}  where ID='{2}'", dateColumnName, dateTableName, rowId);
				RetDate = System.Convert.ToDateTime(cmd.ExecuteScalar().ToString());
				Compare(TestedDate1, RetDate);
			} 
			catch(Exception ex)
			{
