...
guint8 *mono_arch_emit_prolog(MonoCompile *cfg)
{
	...
	cfg->code_size = 40 + signature->param_count * 10;
	...
	MONO_BB_FOR_EACH_INS(basic_block, inst) {
		...
		case OP_LOADI4_MEMBASE:
			if (!SH4_CHECK_RANGE_movl_dispRy(inst->inst_offset)) {
				...
				inst->inst_offset = 0;
				SH4_CFG_DEBUG(4) SH4_DEBUG("coucou: off= 0x%x\n", inst->inst_offset);
				inst->inst_basereg = temp2->dreg;
				/* inst->dreg destination reg is kept */
			}
			SH4_CFG_DEBUG(4) SH4_DEBUG("coucou2: off= 0x%x\n", inst->inst_offset);
			break;
	...
	MONO_BB_FOR_EACH_INS(basic_block, inst) {
		...
		case OP_ICONST:
			...
			if (SH4_CHECK_RANGE_mov_imm(inst->inst_imm)) {
				sh4_mov_imm(cfg, &buffer, inst->inst_imm, inst->dreg);
			} else {
				sh4_cstpool_add(cfg,&buffer,MONO_PATCH_INFO_NONE,
		...
		default:
			...
			/* MD: int_bne_un: */
			/* MD: int_cgt_un: dest:i len:16 */
			/* MD: bne.un: len:4 */
			/* MD: label: len:0 */
			/* MD: compare_imm: src1:i len:4 */
